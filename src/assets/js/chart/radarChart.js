...
function RadarChart() {
   ...
   var options = {
      ...
      axes: {
         display: true,
         threshold: 10,    // radius threshold for hiding
         lineColor: "white",
         lineWidth: "2px",
         fontWidth: "11px",
         fontColor: "black",
...
//                    .style("fill", function(d,i) { return options.axis.interfaceColor(d.value); });
                ...
                update_blobWrapper.enter()
                   .append("g")
                   .attr("class", function(d) { return options.class + "RadarWrapper " + d.key })
                ...
                update_radarStroke.enter()
                   .append("path")
                   .attr("class", function(d){ return options.class + "RadarStroke " + d.key})
                   .attr("d", function(d, i) { return radarLine(d.values); })
                ...
                update_radarCircle.enter()
                   ...
                   .style("fill", function(d, i, j) { return setColor(d, d._i, _data[j]); })
                      ...
                      legend_node.selectAll('.cell')
                        .attr('gen', function(d, i) {
                           if (legend_toggles[d] == true) {
                              var shape = d3.symbol().type(options.legend.toggle).size(150)()
                           } else {
                              var shape = d3.symbol().type(options.legend.symbol).size(150)()
                           }
   ...
   function calcX(value, scale, index) {
      ...
      let output =  radial_calcs.rScale(value ?
                                 value
   ...
   chart.events = function(functions) {
            ...
            if (typeof(options[key]) == 'object') {
              for (var subkey in values[key]) {
                if (subkey in options[key])
                  options[key][subkey] = values[key][subkey]
              }
            } else {
    ...
    chart.margins = function(value) {
        ...
        for (var key in values) {
          if (key in options.margins) options.margins[key] = values[key]
        }
        return chart;
    }
